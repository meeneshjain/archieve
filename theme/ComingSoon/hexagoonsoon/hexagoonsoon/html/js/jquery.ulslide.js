    ...
    $.fn.ulslide = function(settings) {
        ...
        function setNavigator(s_navigator) {
            ...
            pager.click(function(){
                ...
                if ((c < settings['count']) && (c != thisEl.uslCurrent())) {
                    //thisEl.uslCurrent(c);
                    if (c > thisEl.uslCurrent()) settings['direction'] = 'b';
                    else settings['direction'] = 'f';
                    thisEl.uslRefresh(c);
                }
                return false;
            });
        ...
        thisEl.uslStatusbar = function() {		
			...
			function isImageLoaded(img) {
				// ¬о врем€ событи€ load IE и другие браузеры правильно
				// определ€ют состо€ние картинки через атрибут complete.
				// »сключение составл€ют Gecko-based браузеры.
				if (!img.complete) {
					return false;
				}
				// “ем не менее, у них есть два очень полезных свойства: naturalWidth и naturalHeight.
				// ќни дают истинный размер изображени€. ≈сли какртинка еще не загрузилась,
